...
class Menu extends Phaser.State {
	...
	create() {
		...
		this.swipe_label = this.game.add.text(this.game.width * 0.5, this.game.height * 0.7 + 20, 'Click to continue', { font: '200px GreatVibes', fill: '#ffffff' });
		...
		this.timer.add(7000, function () {
			// Swipe
			this.input.onDown.add(this.swipeUp, this);
	...
	swipeUp() {
		if (this.state === MENU_SPLASH) {
			// this.input.onDown.remove(this.swipeUp);
			// this.input.onDown.add(this.startGame, this);
		...
		} else if (this.state === MENU_INSTRUCTIONS) {
			this.startGame();
		}
	}
	...
	startGame() {
		if (this.state === MENU_INSTRUCTIONS) {
			// this.input.onDown.remove(this.startGame);
